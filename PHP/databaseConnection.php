...
function createNewComment($user_id, $comment, $post_id){
    ...
    if(strlen($comment) > 9 && strlen($comment) < 256){
        try{
...
function createNewPost($user_id, $postContent, $title){
    global $connection;
    $message = "De comment is succesvol geplaatst!";
    if(strlen($postContent) > 9 && strlen($postContent) < 256 && strlen($title) > 4 && strlen($title) < 51){
        try{
            $sql = ("INSERT INTO ForumPost (user_id, postname, post, date) VALUES(:user_id, :title, :postContent, GETDATE())");
            $preparedQuary = $connection->prepare($sql);
            $preparedQuary->bindParam(':user_id', $user_id, PDO::PARAM_INT);
